
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Mybookshelve.com | Where shelves are full of books</title>
        <!--- global css with basic styling and favicon -->
        <link rel="stylesheet" href="{{asset ('global.css')}}">
        <link rel="shortcut icon" type="image/x-icon" href={{asset('favicon.ico')}} />
        <link href="https://fonts.googleapis.com/css2?family=Titillium+Web:wght@200;400&display=swap" rel="stylesheet">
    </head>
    <body>
       
     {{-- Navigation section --}}
        
    <div class="stick-nav">
    <ul class="nav-elements">
        <li class="right-elem"></li>
        <li ><a href="/about-us">About us</a></li>
        <li><a href="/products">Products</a></li>
        <li><a href="/contact">Contact us</a></li>
        
        
   
    
        <li class="left-elem"></li>
        <li title="Register"><a href="/register">
            <svg class="bi bi-person-plus" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" d="M11 14s1 0 1-1-1-4-6-4-6 3-6 4 1 1 1 1h10zm-9.995-.944v-.002.002zM1.022 13h9.956a.274.274 0 0 0 .014-.002l.008-.002c-.001-.246-.154-.986-.832-1.664C9.516 10.68 8.289 10 6 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664a1.05 1.05 0 0 0 .022.004zm9.974.056v-.002.002zM6 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4zm3-2a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm4.5 0a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1 0-1H13V5.5a.5.5 0 0 1 .5-.5z"/>
              <path fill-rule="evenodd" d="M13 7.5a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1H14v1.5a.5.5 0 0 1-1 0v-2z"/></a>
            </svg></li>
    
            <li title="Login"><a href="/auth">
            <svg class="bi bi-door-closed" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M3 2a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2zm1 0v13h8V2H4z"/>
                <path d="M7 9a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"/>
                <path fill-rule="evenodd" d="M1 15.5a.5.5 0 0 1 .5-.5h13a.5.5 0 0 1 0 1h-13a.5.5 0 0 1-.5-.5z"/>
              </svg></li>
    
              <li title="Log out" class="hidden"><a href="#logout"><a href="#log_out">
                <svg class="bi bi-box-arrow-in-right" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M8.146 11.354a.5.5 0 0 1 0-.708L10.793 8 8.146 5.354a.5.5 0 1 1 .708-.708l3 3a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0z"/>
                    <path fill-rule="evenodd" d="M1 8a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 1 8z"/>
                    <path fill-rule="evenodd" d="M13.5 14.5A1.5 1.5 0 0 0 15 13V3a1.5 1.5 0 0 0-1.5-1.5h-8A1.5 1.5 0 0 0 4 3v1.5a.5.5 0 0 0 1 0V3a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 .5.5v10a.5.5 0 0 1-.5.5h-8A.5.5 0 0 1 5 13v-1.5a.5.5 0 0 0-1 0V13a1.5 1.5 0 0 0 1.5 1.5h8z"/>
                  </svg></li>
    
                <li title="cart"><a href="/cart">
                <svg class="bi bi-cart4" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M0 2.5A.5.5 0 0 1 .5 2H2a.5.5 0 0 1 .485.379L2.89 4H14.5a.5.5 0 0 1 .485.621l-1.5 6A.5.5 0 0 1 13 11H4a.5.5 0 0 1-.485-.379L1.61 3H.5a.5.5 0 0 1-.5-.5zM3.14 5l.5 2H5V5H3.14zM6 5v2h2V5H6zm3 0v2h2V5H9zm3 0v2h1.36l.5-2H12zm1.11 3H12v2h.61l.5-2zM11 8H9v2h2V8zM8 8H6v2h2V8zM5 8H3.89l.5 2H5V8zm0 5a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm-2 1a2 2 0 1 1 4 0 2 2 0 0 1-4 0zm9-1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm-2 1a2 2 0 1 1 4 0 2 2 0 0 1-4 0z"/>
                </svg></li>

    
            <li title="My favourites" class="hidden"><a href="#fav">
            <svg class="bi bi-heart" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M8 2.748l-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15z"/></a>
              </svg></li>
    
        <li title="Home"><a href="/">
            <svg class="bi bi-house" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" d="M2 13.5V7h1v6.5a.5.5 0 0 0 .5.5h9a.5.5 0 0 0 .5-.5V7h1v6.5a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 2 13.5zm11-11V6l-2-2V2.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5z"/>
            <path fill-rule="evenodd" d="M7.293 1.5a1 1 0 0 1 1.414 0l6.647 6.646a.5.5 0 0 1-.708.708L8 2.207 1.354 8.854a.5.5 0 1 1-.708-.708L7.293 1.5z"/></a>
            </svg></li>
       
    </ul><hr>
    
        </div>
    {{-- End of navigation --}}
    {{-- Header with banner--}}
<div class="header-section">
<img class="header-section" src="{{ asset('mybookshelve.png') }}" alt="banner">
</div>

<div class="welcome-info">
    <div class="text-box">
        <h1>Welcome in the world of books!</h1>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit.
         Illo distinctio quos ducimus impedit deleniti unde, maxime saepe facilis deserunt quia cupiditate doloremque eaque velit quasi, 
        explicabo molestiae? A, reiciendis fuga! Lorem ipsum dolor sit, amet consectetur adipisicing elit. 
        Corporis porro cum culpa pariatur omnis sit excepturi, numquam ab ipsam ducimus, voluptatum eius animi soluta tempore rem exercitationem molestiae quam veniam.
         Lorem ipsum dolor sit amet consectetur adipisicing elit. Laudantium error a, enim voluptatibus voluptas ducimus ab deleniti, 
        repellendus ipsam reiciendis dolor tenetur provident aliquam deserunt labore nam porro minima est?</p><br>
        <form action="/products">
            <input class="show_button" type="submit" value="Check our products" /><br><br>
        </form>
    </div>
    <div class="img-box">
        <img src="{{ asset('library.jpg') }}" alt="library">
    </div>

</div>
<br>

<div class="foot">
    
           
                <h3> Contact info</h3>
                <p>Mybookshelve.com<br>
                Address: Karpinskiej 26,<br>
                City: Poznan<br>
                Post-code: 60-462<br>
                Phone: 794 506 899<br>
                mail: pwroblewski1@edu.cdv.pl<br>
            </p>
             
            </div>
    
        
    <footer>
        © 2020 by Piotr Wroblewski
    </footer>
    
    <script src={{asset ('navscript.js')}}></script>
    </body>
    </html>